...
fn main() {
    ...
    for vec in data{
      let mut tempvec = vec.clone();
      let mut tempres = thread::spawn(move || map_data(&tempvec));
      // 4. Collects the intermediate sums from all the threads
      intermediate_sums.push(tempres.join().unwrap());
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut fxs: Vec<Vec<usize>> = Vec::new();

    for n in 0..num_partitions{
      let mut x1 : Vec<usize> = Vec::new();
      for i in partition_size*n..partition_size*(n+1){
          x1.push(v[i]);
      } 
      fxs.push(x1);
    }
